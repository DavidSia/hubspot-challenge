...
const processMeetings = async (domain, hubId, q) => {
  ...
  while (hasMore) {
    ...
    if (!searchResult) throw new Error('Failed to fetch meetings after 4 retries. Aborting.');
    ...
    // Retrieve associated contacts for each meeting
    const meetingToContactAssociations = await hubspotClient.apiRequest({
      method: 'POST',
    ...
    const enrichedMeetings = meetings.map(meeting => {
      const associatedContact = meetingToContactAssociations.results.find(
        assoc => assoc.from.id === meeting.id
      );

      return {
        ...meeting,
        contactId: associatedContact?.to?.[0]?.id || null,
      };
    });
    ...
    for (const meeting of enrichedMeetings) {
      ...
      if (contactId) {
        ...
        contactEmail = contact?.properties?.email || 'N/A';
      }
    ...
    if (!offsetObject?.after) {
      ...
      offsetObject.after = 0;
      offsetObject.lastModifiedDate = new Date(meetings[meetings.length - 1].properties.hs_lastmodifieddate).valueOf();
    }
  }
